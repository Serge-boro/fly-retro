...
const flip = {
  ...
  update() {
    ...
    for (i in flip2) {
      if (
        Math.abs(flymen2.x + 15 - flip2[i].x_1 - 25) < 60 &&
        Math.abs(flymen2.y + 25 - flip2[i].y_1 - 5) < 60 &&
        Math.abs(flymen2.y - 25 - flip2[i].y_1 + 25) < 100
      ) {
        score.value++
        flip2.splice(i, 1)
        myAnimation()
...
const fon_start = {
  ...
  draw() {
    ...
    ctx.drawImage(
      flymen2_blackimg,
      this.w_men * this.frameX_men,
      this.h_men * this.frameY_men,
      this.w_men,
      this.h_men,
      this.x_men,
      this.y_men,
...
const score = {
  ...
  draw_getReady() {
    ctx.drawImage(
      start_buttonimg,
      this.x_btn_start_getReady,
      this.y_btn_start_getReady,
      this.w_btn_start_getReady,
      this.h_btn_start_getReady
    )
    ctx.drawImage(
      quit_buttonimg,
      this.sX_btn_quit_getReady,
      this.sY_btn_quit_getReady,
      this.w_btn_quit_getReady,
      this.h_btn_quit_getReady,
      this.x_btn_quit_getReady,
      this.y_btn_quit_getReady,
      100,
      80
    )
  },
  ...
  draw_over() {
    ...
    ctx.drawImage(
      quit_buttonimg,
      this.sX_btn_start_over,
      this.sY_btn_start_over,
      this.w_btn_start_over,
      this.h_btn_start_over,
      this.x_btn_start_over,
      this.y_btn_start_over,
      150,
      100
    )
    ctx.drawImage(
...
const litter = {
  ...
  update() {
    ...
    for (i in litter2) {
      if (
        Math.abs(flymen2.x + 15 - litter2[i].x - 25) < 60 &&
        Math.abs(flymen2.y + 25 - litter2[i].y - 5) < 60 &&
        Math.abs(flymen2.y - 25 - litter2[i].y + 25) < 100
      ) {
        litter2.splice(i, 1)
    ...
    for (i in litter_sigh) {
      if (
        Math.abs(flymen2.x + 15 - litter_sigh[i].x - 25) < 60 &&
        Math.abs(flymen2.y + 25 - litter_sigh[i].y - 5) < 60 &&
        Math.abs(flymen2.y - 25 - litter_sigh[i].y + 25) < 100
      ) {
    ...
    for (i in litter_shoe) {
      if (
        Math.abs(flymen2.x + 15 - litter_shoe[i].x - 25) < 60 &&
        Math.abs(flymen2.y + 25 - litter_shoe[i].y - 5) < 60 &&
        Math.abs(flymen2.y - 25 - litter_shoe[i].y + 25) < 100
      ) {
    ...
    for (i in litter_bottle1) {
      if (
        Math.abs(flymen2.x + 15 - litter_bottle1[i].x - 25) < 60 &&
        Math.abs(flymen2.y + 25 - litter_bottle1[i].y - 5) < 60 &&
        Math.abs(flymen2.y - 25 - litter_bottle1[i].y + 25) < 100
      ) {
    ...
    for (i in litter_can) {
      if (
        Math.abs(flymen2.x + 15 - litter_can[i].x - 25) < 60 &&
        Math.abs(flymen2.y + 25 - litter_can[i].y - 5) < 60 &&
        Math.abs(flymen2.y - 25 - litter_can[i].y + 25) < 100
      ) {
    ...
    for (i in fly_bottle1) {
      if (litter_bottle1[i].x < 0) {
        ...
        litter_bottle1[i].y =
          Math.random() * canvas.height - litter_bottle1[i].h
...
canvas.addEventListener('click', function (evt) {
  switch (state.current) {
    ...
    case state.getReady:
      ...
      if (
        clickX >= score.x_btn_start_getReady_button &&
        clickX <=
          score.x_btn_start_getReady_button +
            score.w_btn_start_getReady_button &&
        clickY >= score.y_btn_start_getReady_button &&
        clickY <=
          score.y_btn_start_getReady_button + score.h_btn_start_getReady_button
      ) {
      ...
      if (
        clickX2 >= score.x_btn_quit_getReady_button &&
        clickX2 <=
          score.x_btn_quit_getReady_button + score.w_btn_quit_getReady_button &&
        clickY2 >= score.y_btn_quit_getReady_button &&
        clickY2 <=
          score.y_btn_quit_getReady_button + score.h_btn_quit_getReady_button
      ) {
        windowClose()
      }
      break
    ...
    case state.over:
      ...
      if (
        clickX3 >= score.x_btn_start_over_button &&
        clickX3 <=
          score.x_btn_start_over_button + score.w_btn_start_over_button &&
        clickY3 >= score.y_btn_start_over_button &&
        clickY3 <= score.y_btn_start_over_button + score.h_btn_start_over_button
      ) {
        state.current = state.getReady
      }

      if (
        clickX4 >= score.x_btn_quit_getReady_button &&
        clickX4 <=
          score.x_btn_quit_getReady_button + score.w_btn_quit_getReady_button &&
        clickY4 >= score.y_btn_quit_getReady_button &&
        clickY4 <=
          score.y_btn_quit_getReady_button + score.h_btn_quit_getReady_button
      ) {
        windowClose()
